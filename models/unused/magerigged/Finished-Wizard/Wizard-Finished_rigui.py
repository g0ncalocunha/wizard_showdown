...
blm_rig_id = "qbosngkuy85s"


class BLOP_PT_rigui_qbosngkuy85s(bpy.types.Panel):
	bl_space_type = 'VIEW_3D'
	bl_region_type = 'UI'
	bl_category = 'Item'
	bl_label = "Rig UI"
	bl_idname = "BLOP_PT_rigui_qbosngkuy85s"
	...
	def draw(self, context):
		...
		row = col.row(align=True)
		row.prop(context.active_object.data,'layers', index=12, toggle=True, text='HEAD')

		row = col.row(align=True)
		row.prop(context.active_object.data,'layers', index=11, toggle=True, text='NECK')
		...
		row = col.row(align=True)
		row.prop(context.active_object.data,'layers', index=15, toggle=True, text='ARM-IK.L')
		row.prop(context.active_object.data,'layers', index=16, toggle=True, text='ARM-IK.R')

		row = col.row(align=True)
		row.prop(context.active_object.data,'layers', index=17, toggle=True, text='ARM-FK.L')
		row.prop(context.active_object.data,'layers', index=18, toggle=True, text='ARM-FK.R')
		...
		row = col.row(align=True)
		row.prop(context.active_object.data,'layers', index=19, toggle=True, text='HANDS')

		row = col.row(align=True)
		row.prop(context.active_object.data,'layers', index=24, toggle=True, text='HAT')
		row.prop(context.active_object.data,'layers', index=25, toggle=True, text='STAFF')
		row.prop(context.active_object.data,'layers', index=26, toggle=True, text='SCARF-TWEAKERS')

		row = col.row(align=True)
		row.prop(context.active_object.data,'layers', index=27, toggle=True, text='ROBE-LEGS-TWEAK')


class BLOP_PT_customprops_qbosngkuy85s(bpy.types.Panel):
	bl_category = 'Item'
	bl_label = "Rig Properties"
	bl_idname = "BLOP_PT_customprops_qbosngkuy85s"
	bl_space_type = 'VIEW_3D'
	bl_region_type = 'UI'
...
classes = (BLOP_PT_rigui_qbosngkuy85s, BLOP_PT_customprops_qbosngkuy85s, )
